...
describe("StripeService.handleWebhook", () => {
  ...
  beforeEach(() => {
    ...
    (service as any).stripe.webhooks.constructEvent = jest.fn().mockReturnValue({
  ...
  it("confirms order and marks product sold when buy-now payment is paid", async () => {
    ...
    expect(orderService.confirmOrder).toHaveBeenCalledWith(42, "cs_paid_123", true);

    // Verify payment was linked to order
    expect(paymentService.linkOrderToSession).toHaveBeenCalledWith("cs_paid_123", 42);
  ...
  it("confirms cart orders and marks products sold when cart payment is paid", async () => {
    ...
    expect(orderService.confirmOrder).toHaveBeenCalledWith(10, undefined, false);
    expect(orderService.confirmOrder).toHaveBeenCalledWith(11, undefined, false);
    expect(orderService.confirmOrder).toHaveBeenCalledWith(12, undefined, false);

    // Verify payment was linked to first order
    expect(paymentService.linkOrderToSession).toHaveBeenCalledWith("cs_cart_paid_123", 10);
  ...
  it("ignores a duplicate webhook for an already confirmed order", async () => {
    const orderServiceWithDuplicate = {
      confirmOrder: jest.fn().mockResolvedValue({ id: 42, status: "CONFIRMED" }),
      markProductsSoldForOrder: jest.fn().mockResolvedValue(undefined),
    };
    ...
    (serviceWithDuplicate as any).stripe.webhooks.constructEvent = jest.fn().mockReturnValue({
      type: "checkout.session.completed",
      data: {
        object: {
    ...
    await serviceWithDuplicate.handleWebhook(Buffer.from("raw-body"), "stripe-signature");

    expect(orderServiceWithDuplicate.confirmOrder).toHaveBeenCalledWith(42, "cs_paid_duplicate_123", true);
    expect(orderServiceWithDuplicate.markProductsSoldForOrder).toHaveBeenCalledWith(42);
  });
});
